...
const RegionCard: React.FC<RegionCardProps> = ({ title, countries, icon, color, delay }) => {
  return (
    <motion.div 
    ...
    >
      ...
      <div className="flex items-start gap-5">
        <div className={`flex-shrink-0 flex items-center justify-center rounded-full h-12 w-12 ${color.replace('bg-', 'bg-').replace('500', '100')} text-${color.replace('bg-', '')}`}>
          {icon}
        </div>
        ...
        <div className="flex-grow">
          ...
          <p className="text-gray-600 mb-4">
            Established partnerships with leading manufacturers and retailers across {title}.
          </p>
      ...
      <div className="absolute bottom-0 right-0 h-16 w-16 opacity-5 rounded-full -mb-8 -mr-8 group-hover:opacity-10 transition-opacity duration-300" style={{background: `radial-gradient(circle, ${color.replace('bg-', '')} 0%, transparent 70%)`}}></div>
    </motion.div>
  );
};
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mb-12 sm:mb-16 text-center">
          ...
          <p className="text-neutral-600 max-w-2xl mx-auto text-lg">
            We've established strong partnerships with clients across major textile markets worldwide, delivering quality products and exceptional service to diverse regions.
          </p>
        </div>
